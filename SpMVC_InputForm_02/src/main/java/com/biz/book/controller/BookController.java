...
public class BookController {
	 ...
	 *  매개변수로 값을 받ㄷ을때 String이 아닌 다른 type으로 받으면
	 *  String to type의 형변환을 시도한다.
	 *  예를 들어 VO의 b_price가 int 형이면
	 *  실제로는
	 *  setB-price(Integer.valueOf(b)price) 형식의 코드가 실행될 것이다.
	 *  이떄 b_price에 입력한 값이 숫자로 변환시키는데 문제가 있을경우
	 *  (Exception이 발생할 경우)
	 *  
